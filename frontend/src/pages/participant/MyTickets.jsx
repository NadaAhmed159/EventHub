...
function normalizeBookedTicket(ticket, approvedEvents) {
  ...
  return {
    ...
    description: matchingEvent?.description || 'Your booking details are ready.',
    venue: matchingEvent?.venue || 'Venue TBD',
    date: matchingEvent?.eventDate || matchingEvent?.date || null,
    categoryName: matchingEvent?.categoryName || matchingEvent?.category || 'General',
    organizerName: matchingEvent?.organizerName || 'Event host',
    image: matchingEvent?.image || 'https://picsum.photos/seed/eventhub-ticket/800/500',
...
export default function MyTickets() {
  ...
  const { data: tickets = [], isLoading: ticketsLoading } = useQuery({
    ...
    queryFn: () => ticketService.getParticipantTickets(user.id),
    select: (response) => response.data || [],
  ...
  const bookedTickets = tickets.map((ticket) => normalizeBookedTicket(ticket, approvedEvents));
  const isLoading = eventsLoading || ticketsLoading;
  const totalTickets = bookedTickets.reduce((sum, ticket) => sum + (ticket.quantity || 0), 0);
  const totalSpent = bookedTickets.reduce((sum, ticket) => sum + (ticket.totalPrice || 0), 0);
  const now = new Date();
                ...
                <div style={{ padding: '1.25rem' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '0.25rem', borderTop: '1px solid #f0f0f0' }}>
                    <div>
                      ...
                      <p style={{ margin: 0, color: '#E63946', fontSize: '1.2rem', fontWeight: '800' }}>${(ticket.totalPrice || 0).toFixed(2)}</p>
                    </div>
                    <button
                      type="button"
