...
export default function OrganizerReviews() {
  ...
  const { data: reviews = [], isLoading } = useQuery({
    queryKey: ['organizer-reviews', user?.id],
    queryFn: () => reviewService.getOrganizerReviews(),
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f5f3f0' }}>
      ...
      <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '3rem 1.5rem' }}>
        ...
        <p style={{ color: '#666', marginBottom: '2rem' }}>Feedback from participants who attended your events.</p>
        ...
        {isLoading ? (
          ...
          <div style={{ backgroundColor: '#fff', padding: '2rem', borderRadius: '12px', textAlign: 'center', boxShadow: '0 2px 12px rgba(0,0,0,0.05)' }}>
            <p style={{ color: '#666', fontSize: '1.1rem' }}>No reviews have been submitted for your events yet.</p>
          </div>
        ) : (
